 ...
 * @Last Modified time: 2024-05-03 11:49:37
 */
...
namespace graphics
{
    ...
    Framebuffer::~Framebuffer()
    {
        ...
        if (spec_.samples > 1)
        {
            glDeleteFramebuffers(1, &m_intermediateRendererId);
    ...
    void Framebuffer::draw(Shader &shader)
    {
        ...
        shader.SetVec2("iResolution", glm::vec2(spec_.width, spec_.height));
        glBindVertexArray(quad_VAO_);
        glBindTexture(GL_TEXTURE_2D, screen_texture_id_);
        glDrawArrays(GL_TRIANGLES, 0, 6);
    }

    void Framebuffer::InitFramebufferWithMSAA()
    {
    }

    void Framebuffer::InitFramebuffer()
    {
        m_isError = util::GenFramebuffer(m_rendererId, spec_, color_id_, depth_id_);
    ...
    void Framebuffer::set_quad_VAO()
    {
        ...
        glBufferData(GL_ARRAY_BUFFER, sizeof(c_quadVertices), &c_quadVertices, GL_STATIC_DRAW);
    ...
    void Framebuffer::unbind()
    {
        if (spec_.samples > 1)
        {
            ...
            glBindFramebuffer(GL_DRAW_FRAMEBUFFER, m_intermediateRendererId);
